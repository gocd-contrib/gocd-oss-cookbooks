...
function setup_nexus_configs() {
  if [ "${SKIP_INTERNAL_CONFIG:-}" != "yes" ]; then
    ...
    copy_to_home_dir maven-settings.xml .m2/settings.xml
...
function cmd_mise_env() {
  echo "GITHUB_TOKEN=\$(cat ${GITHUB_TKN_FILE}) CLICOLOR_FORCE=1"
}
...
function cmd_echo_mise_install_globally_from_local() {
  echo "export $(cmd_mise_env) && mise install --yes && mise use --global --yes \$(mise list --local --json | jq -r --arg at @ 'to_entries[] | .key + \$at + .value[].requested_version')"
}
