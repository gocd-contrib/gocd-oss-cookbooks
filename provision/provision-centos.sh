...
function provision() {
  ...
  step install_asdf "v0.13.0" "java" "ruby" "nodejs"

  step install_global_asdf "java" "temurin-17.0.8+101"
  step install_multi_asdf "java" "temurin-17.0.8+101"

  step install_global_asdf "ruby" "3.2.2"
  step install_global_ruby_default_gems

  step install_global_asdf "nodejs" "18.18.0"
  step install_yarn

  step install_maven "$MAVEN_VERSION"
  step install_ant "$ANT_VERSION"

  step install_gauge "1.5.4"
...
function build_gocd() {
  ...
  try su - ${PRIMARY_USER} -c "git clone --depth 1 https://github.com/gocd/gocd /tmp/gocd && \
              cd /tmp/gocd && \
              asdf install && \
              ./gradlew --max-workers 2 compileAll yarnInstall --no-build-cache ${GRADLE_OPTIONS}"
