...
$GOLANG_BOOTSTRAPPER_VERSION='2.37'
$P4_VERSION='25.2'
$ANT_VERSION='1.10.15'
$NANT_VERSION='0.92'
...
# Finds the latest winget package version starting with a specific version prefix.
function Find-WingetPackageLatestVersionPrefix {
    param (
        [Parameter(Mandatory)]
        [string]$PackageName,
    ...
    # Get versions (skip header lines)
    $allVersions = winget show --id $PackageName --versions --exact 2>$null |
    ...
    if (-not $allVersions) {
        Write-Host "Package '$PackageName' not found." -ForegroundColor DarkRed
        return $null
    }
    ...
    if ($filtered) {
        ...
        $latest = $filtered | Sort-Object { [version]$_ } -Descending | Select-Object -First 1
...
# install packages
Winget-Install OpenJS.NodeJS.LTS --version=$(Find-WingetPackageLatestVersionPrefix "OpenJS.NodeJS.LTS" "v${NODEJS_VERSION}")
corepack enable
yarn set version stable
yarn config --no-defaults

Winget-Install EclipseAdoptium.Temurin.${JAVA_VERSION}.JDK

Winget-Install Git.Git Mercurial.Mercurial Slik.Subversion Google.Chrome
git config --global core.autocrlf false

Winget-Install RubyInstallerTeam.RubyWithDevKit.${RUBY_VERSION}
